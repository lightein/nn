...
def main():
    ...
    parser.add_argument('--kword', type=str, default='*.png')
    ...
    for path in data_list:
        pool.apply_async(worker, args=(path, kword, data_root, result_root, crop_sz, step, thres_sz), callback=update)
    pool.close()
    pool.join()
    print('All subprocesses done.')


def worker(path, kword, data_root, result_root, crop_sz, step, thres_sz):
    path = path.replace('\\', '/')
    ...
    for img_path in img_paths:
        ...
        for x in h_space:
            for y in w_space:
                ...
                cv2.imwrite(os.path.join(save_folder, img_name.replace(ext, '_s{:03d}'.format(index) + ext)), crop_img)
